...
\titulo{Research Internships Abroad Proposal:\\ Evaluating the practical use of fault localization techniques}
...
Debugging is an inherent task in the software development process.
Developers spend a significant amount of time searching for faults in program 
code excerpts. In practice, the debugging activity continues to be carried out 
manually. Developers inspect the code using symbolic debuggers or including 
print statements in the code.
Automating the fault localization process can collaborate to reduce the time spent 
in the software development and to improve the software quality.
In the recent years, several fault localization techniques have been proposed. 
Most of the techniques generate a list with code excerpts more likely to 
be faulty. Thus, developers can inspect such list to search for faults.
However, practical evaluations with these techniques are needed to obtain a 
better understanding about how the developers use them.
Recent studies indicate that developers have an ``effort budget'' that they are willing 
to spend using such techniques. If the fault is not found inspecting an certain amount 
of code (the budget), they often use another personal strategy to inspect the code. 
Moreover, the lists suggested by fault localization techniques are not followed in the 
same presented sequence.
The evaluation of the fault localization effort by developers when using fault 
localization techniques is an important question to understand their practical usefulness 
and how to improve them to be used in practice.
...
and multiple faults.
The research internship will be carried out at the Georgia Institute of Technology under 
the supervision of the Professor Alessandro Orso.
% Our aim is to collaborate with fault localization in order to allow its practical use in 
% industrial settings.
